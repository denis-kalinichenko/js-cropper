...
describe("Image component",() => {
    ...
    it("creates Image element", () => {
        image = new Image();
        expect(image._node.nodeType).to.equal(Node.ELEMENT_NODE);
        expect(image._node).to.be.an.instanceof(window.HTMLElement);
    });
    ...
    it("has load method, which loads an image by provided URL or path and returns this", () => {
        ...
        return image.load(url).then((loadedImage) => {
            expect(loadedImage._node.src).to.equal(url);
    ...
    it("has checkFormat method, which check an image format (landscape, portrait or square) and return format", () => {
        ...
        image._node.height = 600;
        expect(image._checkFormat()).to.equal("portrait");

        image = new Image();
        image._node.width = 330;
        image._node.height = 330;
        expect(image._checkFormat()).to.equal("square");

        image = new Image();
        image._node.width = 690;
        image._node.height = 330;
    ...
    it("has scaleToFit method, which scale image to fit Frame", () => {
        ...
        expect(image._scale).to.equal(frame.getRect().size.w / image._node.width);
    ...
    it("has getSize method, which Returns Size object, which contain weight and height", () => {
        ...
        expect(image.getSize().w).to.equal(image._node.width * image._scale);
        expect(image.getSize().h).to.equal(image._node.height * image._scale);
    });
